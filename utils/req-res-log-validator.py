...
1. Goes over req-res files, generated by sider-servers, spawned by the testsuite (see logreqres.c)
2. For each request-response pair, it validates the response against the request's reply_schema (obtained from COMMAND DOCS)

This script spins up a sider-server and a sider-cli in order to obtain COMMAND DOCS.

In order to use this file you must run the sider testsuite with the following flags:
...
class Request(object):
    """
    This class represents a Sider request (AKA command, argv)
    """
...
class Response(object):
    """
    This class represents a Sider response in RESP3
    """
    def __init__(self, f, line_counter):
        ...
        if line[0] == '+':
            ...
            for i in range(count):
                ...
                # Sider allows fields to be non-strings but JSON doesn't.
...
def fetch_schemas(cli, port, args, docs):
    sider_proc = subprocess.Popen(args, stdout=subprocess.PIPE)

    while True:
        try:
            print('Connecting to Sider...')
            r = sider.Sider(port=port)
            r.ping()
            break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--server', type=str, default='%s/sider-server' % srcdir)
    ...
    # Fetch schemas from a Sider instance
    print('Starting Sider server')
    sider_args = [args.server, '--port', str(args.port)]
    for module in args.module:
        sider_args += ['--loadmodule', 'tests/modules/%s.so' % module]

    fetch_schemas(args.cli, args.port, sider_args, docs)

    # Fetch schemas from a sentinel
    print('Starting Sider sentinel')
