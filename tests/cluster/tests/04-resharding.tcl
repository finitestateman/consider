...
set numkeys 50000
set numops 200000
set start_node_port [get_instance_attrib sider 0 port]
set cluster [sider_cluster 127.0.0.1:$start_node_port]
if {$::tls} {
    # setup a non-TLS cluster client to the TLS cluster
    set plaintext_port [get_instance_attrib sider 0 plaintext-port]
    set cluster_plaintext [sider_cluster 127.0.0.1:$plaintext_port 0]
    ...
    for {set j 0} {$j < $numops} {incr j} {
        ...
        if {$j >= $numops/2 && $tribpid eq {}} {
            ...
            set tribpid [lindex [exec \
                ../../../src/sider-cli --cluster reshard \
                127.0.0.1:[get_instance_attrib sider 0 port] \
                --cluster-from all \
                --cluster-to $target \
                --cluster-slots 100 \
                --cluster-yes \
        ...
        # This way we are able to stress Lua -> Sider command invocation
        ...
        if {$listid % 3 == 0} {
            ...
            $cluster eval {sider.call("rpush",KEYS[1],ARGV[1])} 1 $key $ele
        }
...
test "Verify $numkeys keys for consistency with logical content" {
    # Check that the Sider Cluster content matches our logical content.
...
test "Terminate and restart all the instances" {
    foreach_sider_id id {
        ...
        kill_instance sider $id
        restart_instance sider $id
    }
}
...
test "Verify slaves consistency" {
    ...
    foreach_sider_id id {
        ...
        set masterport [get_instance_attrib sider $id port]
        set masterdigest [R $id debug digest]
        foreach_sider_id sid {
