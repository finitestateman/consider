...
test "Verify information about the shards" {
    ...
    for {set ref 0} {$ref < $::cluster_master_nodes + $::cluster_replica_nodes} {incr ref} {
        for {set i 0} {$i < $::cluster_master_nodes + $::cluster_replica_nodes} {incr i} {
            ...
            if {$::tls} {
                assert_equal [get_instance_attrib sider $i plaintext-port] [dict get [get_node_info_from_shard [lindex $ids $i] $ref "node"] port]
                assert_equal [get_instance_attrib sider $i port] [dict get [get_node_info_from_shard [lindex $ids $i] $ref "node"] tls-port]
            } else {
                assert_equal [get_instance_attrib sider $i port] [dict get [get_node_info_from_shard [lindex $ids $i] $ref "node"] port]
            }
...
test "Regression test for a crash when calling SHARDS during handshake" {
    ...
    R 19 cluster meet 127.0.0.1 [get_instance_attrib sider 0 port]
...
test "CLUSTER MYSHARDID reports same shard id after shard restart" {
    ...
    for {set i 0} {$i < 8} {incr i 4} {
        ...
        kill_instance sider $i
        wait_for_condition 50 100 {
            [instance_is_killed sider $i]
        } else {
            fail "instance $i is not killed"
        }
    }
    for {set i 0} {$i < 8} {incr i 4} {
        restart_instance sider $i
    }
    assert_cluster_state ok
    for {set i 0} {$i < 8} {incr i 4} {
