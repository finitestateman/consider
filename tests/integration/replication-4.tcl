...
start_server {tags {"repl external:skip"}} {
    start_server {} {
        ...
        $master FUNCTION load replace {#!lua name=test
            sider.register_function{function_name='f_default_flags', callback=function(keys, args) return sider.call('get',keys[1]) end, flags={}}
            sider.register_function{function_name='f_no_writes', callback=function(keys, args) return sider.call('get',keys[1]) end, flags={'no-writes'}}
        }
        ...
        test {With min-slaves-to-write (1,3): master should be writable} {
            ...
            assert_equal OK [$master eval "return sider.call('set','foo',12345)" 0]
        }
        ...
        test {With min-slaves-to-write (2,3): master should not be writable} {
            ...
            assert_error "*NOREPLICAS*" {$master eval "sider.call('set','foo','bar')" 0}
        }
        ...
        test {With not enough good slaves, read in Lua script is still accepted} {
            $master config set min-slaves-max-lag 3
            $master config set min-slaves-to-write 1
            $master eval "sider.call('set','foo','bar')" 0

            $master config set min-slaves-to-write 2
            $master eval "return sider.call('get','foo')" 0
        } {bar}
        ...
        test {With min-slaves-to-write: master not writable with lagged slave} {
            ...
            assert_error "*NOREPLICAS*" {$master eval "return sider.call('set','foo',12345)" 0}
