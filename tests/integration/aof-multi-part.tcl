...
tags {"external:skip"} {
    ...
    # In order to test the loading logic of sider under different combinations of manifest and AOF.
    # We will manually construct the manifest file and AOF, and then start sider to verify whether
    # the sider behavior is as expected.
    ...
    test {Multi Part AOF can start when no aof and no manifest} {
        start_server_aof [list dir $server_path] {
            ...
            set client [sider [dict get $srv host] [dict get $srv port] 0 $::tls]
    ...
    test {Multi Part AOF can load data from old version sider (rdb preamble no)} {
    ...
    test {Multi Part AOF can load data from old version sider (rdb preamble yes)} {
    ...
    test {Multi Part AOF can upgrade when when two sider share the same server dir} {
        create_aof $server_path $aof_old_name_old_path {
            append_to_aof [formatCommand set k1 v1]
            append_to_aof [formatCommand set k2 v2]
            append_to_aof [formatCommand set k3 v3]
        ...
        start_server_aof [list dir $server_path] {
            set sider1 [sider [dict get $srv host] [dict get $srv port] 0 $::tls]

            start_server [list overrides [list dir $server_path appendonly yes appendfilename appendonly.aof2]] {
                set sider2 [sider [srv host] [srv port] 0 $::tls]

                test "Multi Part AOF can upgrade when when two sider share the same server dir (sider1)" {
                    wait_done_loading $sider1
                    assert_equal v1 [$sider1 get k1]
                    assert_equal v2 [$sider1 get k2]
                    assert_equal v3 [$sider1 get k3]

                    assert_equal 0 [$sider1 exists k4]
                    assert_equal 0 [$sider1 exists k5]
                    assert_equal 0 [$sider1 exists k6]
                    ...
                    $sider1 bgrewriteaof
                    waitForBgrewriteaof $sider1

                    assert_equal OK [$sider1 set k v]
                    ...
                    set d1 [$sider1 debug digest]
                    $sider1 debug loadaof
                    set d2 [$sider1 debug digest]
                    assert {$d1 eq $d2}
                }

                test "Multi Part AOF can upgrade when when two sider share the same server dir (sider2)" {
                    wait_done_loading $sider2

                    assert_equal 0 [$sider2 exists k1]
                    assert_equal 0 [$sider2 exists k2]
                    assert_equal 0 [$sider2 exists k3]

                    assert_equal v4 [$sider2 get k4]
                    assert_equal v5 [$sider2 get k5]
                    assert_equal v6 [$sider2 get k6]
                    ...
                    $sider2 bgrewriteaof
                    waitForBgrewriteaof $sider2

                    assert_equal OK [$sider2 set k v]
                    ...
                    set d1 [$sider2 debug digest]
                    $sider2 debug loadaof
                    set d2 [$sider2 debug digest]
                    assert {$d1 eq $d2}
                }
            }
        }
    ...
    test {Multi Part AOF can handle appendfilename contains whitespaces} {
        start_server [list overrides [list appendonly yes appendfilename "\" file seq \\n\\n.aof \""]] {
            set dir [get_sider_dir]
            set aof_manifest_name [format "%s/%s/%s%s" $dir "appendonlydir" " file seq \n\n.aof " $::manifest_suffix]
            set sider [sider [srv host] [srv port] 0 $::tls]

            assert_equal OK [$sider set k1 v1]

            $sider bgrewriteaof
            waitForBgrewriteaof $sider
            ...
            set d1 [$sider debug digest]
            $sider debug loadaof
            set d2 [$sider debug digest]
            assert {$d1 eq $d2}
        }
    ...
    test {Multi Part AOF can create BASE (RDB format) when sider starts from empty} {
        start_server_aof [list dir $server_path] {
            set client [sider [dict get $srv host] [dict get $srv port] 0 $::tls]
            wait_done_loading $client
    ...
    test {Multi Part AOF can create BASE (AOF format) when sider starts from empty} {
    ...
    # Test Part 2
    #
    # To test whether the AOFRW behaves as expected during the sider run.
    # We will start sider first, then perform pressure writing, enable and disable AOF, and manually
