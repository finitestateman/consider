start_server {tags {"expire"}} {
    ...
    test {Sider should actively expire keys incrementally} {
        r flushdb
        r psetex key1 500 a
        r psetex key2 500 a
        r psetex key3 500 a
        assert_equal 3 [r dbsize]
        # Sider expires random keys ten times every second so we are
    ...
    test {Sider should lazy expire keys} {
        r flushdb
    ...
    test {All TTL in commands are propagated as absolute timestamp in replication stream} {
        ...
        # See: https://github.com/sider/sider/issues/8433
    ...
    test {Sider should not propagate the read command on lazy expire} {
        r debug set-active-expire 0
        r flushall ; # Clean up keyspace to avoid interference by keys from other tests
        r set foo bar PX 1
        set repl [attach_to_replication_stream]
