start_server {tags {"tls"}} {
    if {$::tls} {
        ...
        test {TLS: Not accepting non-TLS connections on a TLS port} {
            set s [sider [srv 0 host] [srv 0 port]]
            catch {$s PING} e
            set e
        } {*I/O error*}
        ...
        test {TLS: Verify tls-protocols behaves as expected} {
            ...
            set s [sider [srv 0 host] [srv 0 port] 0 1 {-tls1.2 0}]
            catch {$s PING} e
            assert_match {*I/O error*} $e

            set s [sider [srv 0 host] [srv 0 port] 0 1 {-tls1.2 1}]
            catch {$s PING} e
            assert_match {PONG} $e
        ...
        test {TLS: Verify tls-ciphers behaves as expected} {
            ...
            set s [sider [srv 0 host] [srv 0 port] 0 1 {-cipher "-ALL:AES128-SHA256"}]
            catch {$s PING} e
            assert_match {*I/O error*} $e

            set s [sider [srv 0 host] [srv 0 port] 0 1 {-cipher "-ALL:AES256-SHA256"}]
            catch {$s PING} e
            assert_match {PONG} $e
        ...
        test {TLS: Verify tls-prefer-server-ciphers behaves as expected} {
            ...
            set s [sider [srv 0 host] [srv 0 port] 0 1 {-cipher "AES256-SHA256:AES128-SHA256"}]
            catch {$s PING} e
            assert_match {PONG} $e
        ...
        test {TLS: Verify tls-cert-file is also used as a client cert if none specified} {
            ...
            set sider_crt [format "%s/tests/tls/sider.crt" [pwd]]
            set sider_key [format "%s/tests/tls/sider.key" [pwd]]

            start_server [list overrides [list tls-cert-file $sider_crt tls-key-file $sider_key] \
        ...
        test {TLS: switch between tcp and tls ports} {
            ...
            # TLS
            set rd [sider [srv 0 host] $srv_port 0 1]
            $rd PING
            ...
            set rd [sider [srv 0 host] $srv_port 0 0]
            $rd PING
