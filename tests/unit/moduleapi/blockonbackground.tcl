...
start_server {tags {"modules"}} {
    ...
    test { blocked clients time tracking - check blocked command that uses SiderModule_BlockedClientMeasureTimeStart() is tracking background time} {
        r slowlog reset
    ...
    test { blocked clients time tracking - check blocked command that uses SiderModule_BlockedClientMeasureTimeStart() is tracking background time even in timeout } {
        r slowlog reset
        r config set slowlog-log-slower-than 200000
        if {!$::no_latency} {
            assert_equal [r slowlog len] 0
    ...
    test { blocked clients time tracking - check blocked command with multiple calls SiderModule_BlockedClientMeasureTimeStart()  is tracking the total background time } {
        r slowlog reset
        r config set slowlog-log-slower-than 200000
        if {!$::no_latency} {
            assert_equal [r slowlog len] 0
    ...
    test { blocked clients time tracking - check blocked command without calling SiderModule_BlockedClientMeasureTimeStart() is not reporting background time } {
        r slowlog reset
        r config set slowlog-log-slower-than 200000
        if {!$::no_latency} {
            assert_equal [r slowlog len] 0
