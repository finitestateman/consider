...
start_server {tags {"modules"}} {
    ...
    test {module auth during blocking module auth} {
        ...
        set rd_two [sider_deferring_client]
    ...
    test {Disabling Sider User during blocking module auth} {
        r config resetstat
        r acl setuser foo >pwd on ~* &* +@all
        set rd [sider_deferring_client]

        # Attempt blocking module auth and disable the Sider user while module auth is in progress.
        $rd AUTH foo pwd
        wait_for_blocked_clients_count 1
        r acl setuser foo >pwd off ~* &* +@all
    ...
    test {test RM_RegisterAuthCallback Module API during blocking module auth} {
        ...
        # Start the module auth attempt with the standard Sider auth password for the user. This
        # will result in all module auth cbs attempted and then standard Sider auth will be tried.
