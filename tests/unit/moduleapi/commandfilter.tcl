...
start_server {tags {"modules"}} {
    ...
    test {Command Filter can call SiderModule_CommandFilterArgDelete} {
        r rpush mylist elem1 @delme elem2
        r lrange mylist 0 -1
    } {elem1 elem2}

    test {Command Filter can call SiderModule_CommandFilterArgInsert} {
        r del mylist
    ...
    test {Command Filter can call SiderModule_CommandFilterArgReplace} {
        r del mylist
    ...
    test {Command Filter applies on Lua sider.call()} {
        r del log-key
        r eval "sider.call('ping', '@log')" 0
        r lrange log-key 0 -1
    } "{ping @log}"

    test {Command Filter applies on Lua sider.call() that calls a module} {
        r del log-key
        r eval "sider.call('commandfilter.ping')" 0
        r lrange log-key 0 -1
    } "{ping @log}"
...
test {RM_CommandFilterArgInsert and script argv caching} {
    ...
    start_server {tags {"modules"}} {
        ...
        r del mylist
        # command with 6 args
        r eval {sider.call('rpush', KEYS[1], 'elem1', 'elem2', 'elem3', 'elem4')} 1 mylist
        # command with 3 args that is changed to 4
        r eval {sider.call('rpush', KEYS[1], '@insertafter')} 1 mylist
