# Primitive tests on cluster-enabled sider using sider-cli
...
start_multiple_servers 3 [list overrides $base_conf] {
    ...
    set node3_rd [sider_deferring_client -2]

    test {Create 3 node cluster} {
        exec src/sider-cli --cluster-yes --cluster create \
    ...
    test "Perform a Resharding" {
        exec src/sider-cli --cluster-yes --cluster reshard 127.0.0.1:[srv -2 port] \
    ...
    test "Wait for cluster to be stable" {
        ...
        wait_for_condition 1000 50 {
            [catch {exec src/sider-cli --cluster check 127.0.0.1:[srv 0 port]}] == 0 &&
            [catch {exec src/sider-cli --cluster check 127.0.0.1:[srv -1 port]}] == 0 &&
            [catch {exec src/sider-cli --cluster check 127.0.0.1:[srv -2 port]}] == 0 &&
            [CI 0 cluster_state] eq {ok} &&
            [CI 1 cluster_state] eq {ok} &&
            [CI 2 cluster_state] eq {ok}
        } else {
    ...
    set node1_rd [sider_deferring_client 0]
    ...
    test "use previous hostip in \"cluster-preferred-endpoint-type unknown-endpoint\" mode" {
        ...
        # when sider-cli not in cluster mode, return MOVE with empty host
        ...
        assert_equal "[exec src/sider-cli -h 127.0.0.1 -p [srv 0 port] -c set foo bar]" {OK}
        assert_match "[exec src/sider-cli -h 127.0.0.1 -p [srv 0 port] -c get foo]" {bar}
...
# Test sider-cli -- cluster create, add-node, call.
# Test that functions are propagated on add-node
start_multiple_servers 5 [list overrides $base_conf] {

    set node4_rd [sider_client -3]
    set node5_rd [sider_client -4]

    test {Functions are added to new node on sider-cli cluster add-node} {
        exec src/sider-cli --cluster-yes --cluster create \
                           127.0.0.1:[srv 0 port] \
                           127.0.0.1:[srv -1 port] \
                           127.0.0.1:[srv -2 port]
        ...
        exec src/sider-cli --cluster-yes --cluster call 127.0.0.1:[srv 0 port] \
                           FUNCTION LOAD {#!lua name=TEST
                               sider.register_function('test', function() return 'hello' end)
                           }

        # adding node to the cluster
        exec src/sider-cli --cluster-yes --cluster add-node \
...
# Test sider-cli --cluster create, add-node.
# Test that one slot can be migrated to and then away from the new node.
test {Migrate the last slot away from a node using sider-cli} {
    start_multiple_servers 4 [list overrides $base_conf] {
        ...
        # Insert some data
        assert_equal OK [exec src/sider-cli -c -p [srv 0 port] SET foo bar]
        set slot [exec src/sider-cli -c -p [srv 0 port] CLUSTER KEYSLOT foo]
        ...
        set owner_r [sider $owner_host $owner_port 0 $::tls]
        set owner_id [$owner_r CLUSTER MYID]

        # Move slot to new node using plain Sider commands
        ...
        wait_for_condition 1000 50 {
            ...
            [catch {exec src/sider-cli --cluster check 127.0.0.1:[srv -1 port]}] == 0 &&
            [catch {exec src/sider-cli --cluster check 127.0.0.1:[srv -2 port]}] == 0 &&
            [catch {exec src/sider-cli --cluster check 127.0.0.1:[srv -3 port]}] == 0 &&
            [CI 0 cluster_state] eq {ok} &&
            [CI 1 cluster_state] eq {ok} &&
            [CI 2 cluster_state] eq {ok} &&
            [CI 3 cluster_state] eq {ok}
        ...
        # Move the only slot back to original node using sider-cli
        exec src/sider-cli --cluster reshard 127.0.0.1:[srv -3 port] \
...
# Test sider-cli --cluster create, add-node with cluster-port.
...
start_server [list overrides [list cluster-enabled yes cluster-node-timeout 1 cluster-port [find_available_port $::baseport $::portcount]]] {
    ...
    set node1_rd [sider_client 0]
    set node2_rd [sider_client -1]
    set node3_rd [sider_client -2]
    set node4_rd [sider_client -3]
    set node5_rd [sider_client -4]

    test {sider-cli --cluster create with cluster-port} {
        exec src/sider-cli --cluster-yes --cluster create \
                           127.0.0.1:[srv 0 port] \
                           127.0.0.1:[srv -1 port] \
                           127.0.0.1:[srv -2 port]
    ...
    test {sider-cli --cluster add-node with cluster-port} {
