...
test {CONFIG SET port number} {
    start_server {} {
        ...
        set rd [sider [srv 0 host] [srv 0 port] 0 $::tls]
        $rd CONFIG SET $port_cfg $avail_port
        $rd close
        set rd [sider [srv 0 host] $avail_port 0 $::tls]
        $rd PING
...
start_server {config "minimal.conf" tags {"external:skip"}} {
    test {Default bind address configuration handling} {
        ...
        # accordingly.
        # (it seems that sidercli_exec behaves differently in RESP3, possibly
        # because CONFIG GET returns a dict instead of a list so sider-cli emits
        # it in a single line)
        if {$::force_resp3} {
            assert_equal {{bind }} [sidercli_exec 0 config get bind]
        } else {
            assert_equal {bind {}} [sidercli_exec 0 config get bind]
        }
        ...
        assert_equal {OK} [sidercli_exec 0 config set bind *]
        reconnect 0
        r ping
    } {PONG}
    ...
    test {Protected mode works as expected} {
        ...
        if {$myaddr != "" && ![string match {127.*} $myaddr]} {
            ...
            set r2 [sider $myaddr [srv 0 "port"] 0 $::tls]
