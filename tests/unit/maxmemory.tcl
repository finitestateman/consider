...
start_server {tags {"maxmemory external:skip"}} {
    ...
    foreach policy {
        ...
        test "maxmemory - only allkeys-* should remove non-volatile keys ($policy)" {
            ...
            # Otherwise we should see an error reported by Sider.
            set err 0
            for {set j 0} {$j < $numkeys} {incr j} {
    ...
    foreach policy {
        ...
        test "maxmemory - policy $policy should only remove volatile keys." {
            ...
            # We expect Sider to evict only volatile keys in order to make
            # space.
            set err 0
            for {set j 0} {$j < $numkeys} {incr j} {
...
proc test_slave_buffers {test_name cmd_count payload_len limit_memory pipeline} {
    start_server {tags {"maxmemory external:skip"}} {
        ...
        test "$test_name" {
            ...
            set rd_slave [sider_deferring_client]
            pause_process $slave_pid

            # send some 10mb worth of commands that don't increase the memory usage
            if {$pipeline == 1} {
                set rd_master [sider_deferring_client -1]
...
# so we need to use a pipeline (sider_deferring_client)
...
start_server {tags {"maxmemory external:skip"}} {
    test {Don't rehash if used memory exceeds maxmemory after rehash} {
        ...
        r set k2 v2
        # There must be 4098 keys because sider doesn't evict keys.
        r dbsize
    } {4098}
}
