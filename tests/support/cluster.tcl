# Tcl sider cluster client as a wrapper of sider.rb.
# Copyright (C) 2014 Salvatore Sanfilippo
# Released under the BSD license like Sider itself
#
# Example usage:
#
# set c [sider_cluster {127.0.0.1:6379 127.0.0.1:6380}]
# $c set foo
# $c get foo
# $c close

package require Tcl 8.5
package provide sider_cluster 0.1

namespace eval sider_cluster {}
set ::sider_cluster::internal_id 0
set ::sider_cluster::id 0
array set ::sider_cluster::startup_nodes {}
array set ::sider_cluster::nodes {}
array set ::sider_cluster::slots {}
array set ::sider_cluster::tls {}

# List of "plain" commands, which are commands where the sole key is always
# the first argument.
set ::sider_cluster::plain_commands {
...
proc sider_cluster {nodes {tls -1}} {
    set id [incr ::sider_cluster::id]
    set ::sider_cluster::startup_nodes($id) $nodes
    set ::sider_cluster::nodes($id) {}
    set ::sider_cluster::slots($id) {}
    set ::sider_cluster::tls($id) [expr $tls == -1 ? $::tls : $tls]
    set handle [interp alias {} ::sider_cluster::instance$id {} ::sider_cluster::__dispatch__ $id]
...
# list of nodes ::sider_cluster::nodes($id) with an hash mapping node
# ip:port to a representation of the node (another hash), and finally
# maps ::sider_cluster::slots($id) with an hash mapping slot numbers
# to node IDs.
#
# This function is called when a new Sider Cluster client is initialized
# and every time we get a -MOVED redirection error.
proc ::sider_cluster::__method__refresh_nodes_map {id} {
    # Contact the first responding startup node.
    set idx 0; # Index of the node that will respond.
    set errmsg {}
    foreach start_node $::sider_cluster::startup_nodes($id) {
        set ip_port [lindex [split $start_node @] 0]
        lassign [split $ip_port :] start_host start_port
        set tls $::sider_cluster::tls($id)
        if {[catch {
            set r {}
            set r [sider $start_host $start_port 0 $tls]
    ...
    if {$idx == [llength $::sider_cluster::startup_nodes($id)]} {
    ...
    foreach line [split $nodes_descr "\n"] {
        ...
        set link {}
        set tls $::sider_cluster::tls($id)
        catch {set link [sider $host $port 0 $tls]}
        ...
        lappend ::sider_cluster::startup_nodes($id) $addr
    }

    # Close all the existing links in the old nodes map, and set the new
    # map as current.
    foreach n $::sider_cluster::nodes($id) {
        catch {
            [dict get $n link] close
        }
    }
    set ::sider_cluster::nodes($id) $nodes
    ...
    dict for {addr node} $nodes {
        foreach slotrange [dict get $node slots] {
            ...
            for {set j $start} {$j <= $end} {incr j} {
                dict set ::sider_cluster::slots($id) $j $addr
            }
        }
    }

    # Only retain unique entries in the startup nodes list
    set ::sider_cluster::startup_nodes($id) [lsort -unique $::sider_cluster::startup_nodes($id)]
}

# Free a sider_cluster handle.
proc ::sider_cluster::__method__close {id} {
    catch {
        set nodes $::sider_cluster::nodes($id)
        dict for {addr node} $nodes {
            catch {
    ...
    catch {unset ::sider_cluster::startup_nodes($id)}
    catch {unset ::sider_cluster::nodes($id)}
    catch {unset ::sider_cluster::slots($id)}
    catch {unset ::sider_cluster::tls($id)}
    catch {interp alias {} ::sider_cluster::instance$id {}}
}

proc ::sider_cluster::__method__masternode_for_slot {id slot} {
    # Get the node mapped to this slot.
    set node_addr [dict get $::sider_cluster::slots($id) $slot]
    if {$node_addr eq {}} {
        error "No mapped node for slot $slot."
    }
    return [dict get $::sider_cluster::nodes($id) $node_addr]
}

proc ::sider_cluster::__method__masternode_notfor_slot {id slot} {
    # Get a node that is not mapped to this slot.
    set node_addr [dict get $::sider_cluster::slots($id) $slot]
    set addrs [dict keys $::sider_cluster::nodes($id)]
    foreach addr [lshuffle $addrs] {
        set node [dict get $::sider_cluster::nodes($id) $addr]
...
proc ::sider_cluster::__dispatch__ {id method args} {
    if {[info command ::sider_cluster::__method__$method] eq {}} {
        # Get the keys from the command.
        set keys [::sider_cluster::get_keys_from_command $method $args]
        if {$keys eq {}} {
            error "Sider command '$method' is not supported by sider_cluster."
        }

        # Resolve the keys in the corresponding hash slot they hash to.
        set slot [::sider_cluster::get_slot_from_keys $keys]
        if {$slot eq {}} {
        ...
        while {[incr retry -1]} {
            ...
            set node [dict get $::sider_cluster::nodes($id) $node_addr]
            ...
            if {[catch {$link $method {*}$args} e]} {
                if {$link eq {} || \
                    ...
                    # MOVED redirection.
                    ::sider_cluster::__method__refresh_nodes_map $id
                    set node_addr [dict get $::sider_cluster::slots($id) $slot]
                    continue
        ...
        error "Too many redirections or failures contacting Sider Cluster."
    } else {
        uplevel 1 [list ::sider_cluster::__method__$method $id] $args
    }
}

proc ::sider_cluster::get_keys_from_command {cmd argv} {
    set cmd [string tolower $cmd]
    # Most Sider commands get just one key as first argument.
    if {[lsearch -exact $::sider_cluster::plain_commands $cmd] != -1} {
...
# The CRC parameters are described in the Sider Cluster specification.
set ::sider_cluster::XMODEMCRC16Lookup {
...
proc ::sider_cluster::crc16 {s} {
    set s [encoding convertto ascii $s]
    set crc 0
    foreach char [split $s {}] {
        scan $char %c byte
        set crc [expr {(($crc<<8)&0xffff) ^ [lindex $::sider_cluster::XMODEMCRC16Lookup [expr {(($crc>>8)^$byte) & 0xff}]]}]
    }
    return $crc
}

# Hash a single key returning the slot it belongs to, Implemented hash
# tags as described in the Sider Cluster specification.
proc ::sider_cluster::hash {key} {
...
# signal that the command can't be run in Sider Cluster.
proc ::sider_cluster::get_slot_from_keys {keys} {
    set slot {}
    foreach k $keys {
        set s [::sider_cluster::hash $k]
        if {$slot eq {}} {
            set slot $s
        } elseif {$slot != $s} {
            return {} ; # Error
