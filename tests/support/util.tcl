...
proc find_valgrind_errors {stderr on_termination} {
    ...
    # Look for the absence of a leak free summary (happens when sider isn't terminated properly).
...
# of seconds to the specified Sider instance.
...
# of ops to the specified Sider instance.
...
# to which Sider instance to write the keys.
...
proc generate_fuzzy_traffic_on_key {key duration} {
    ...
    while {([clock seconds]-$start_time) < $duration} {
        ...
        # get the command details from sider
        if { [ catch {
            set cmd_info [lindex [r command info $cmd] 0]
        } err ] } {
            # if we failed, it means sider crashed after the previous command
            return $sent
        }
...
proc read_from_aof {fp} {
    ...
    for {set j 0} {$j < $count} {incr j} {
        read $fp 1
        set arg [::sider::sider_bulk_read $fp]
...
proc get_nonloopback_client {} {
    return [sider [get_nonloopback_addr] [srv 0 "port"] 0 $::tls]
}
...
# Utility function to write big argument into sider client connection
