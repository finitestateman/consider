...
proc start_server_error {config_file error} {
    set err {}
    append err "Can't start the Sider server\n"
...
proc spawn_server {config_file stdout stderr args} {
    set cmd [list src/sider-server $config_file]
    set args {*}$args
    if {[llength $args] > 0} {
        lappend cmd {*}$args
    }
...
proc start_server {options {code undefined}} {
    ...
    if {$::tls} {
        if {$::tls_module} {
            lappend config_lines [list "loadmodule" [format "%s/src/sider-tls.so" [pwd]]]
        }
        ...
        dict set config "tls-dh-params-file" [format "%s/tests/tls/sider.dh" [pwd]]
    ...
    set config_file [tmpfile sider.conf]
